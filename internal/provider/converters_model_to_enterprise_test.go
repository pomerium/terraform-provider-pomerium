...
func TestModelToEnterpriseConverter(t *testing.T) {
	...
	t.Run("Route", func(t *testing.T) {
		...
		t.Run("IdentityProviders", func(t *testing.T) {
			...
			var diagnostics diag.Diagnostics
			actual := provider.NewModelToEnterpriseConverter(&diagnostics).Route(provider.RouteModel{
				IdentityProviders: types.SetValueMust(types.StringType, []attr.Value{
	...
	t.Run("RecordingDataSource", func(t *testing.T) {
		...
		for _, tc := range []struct {
			name string
			src  provider.RecordingDataSourceModel
			want *pb.Datasource
		}{
			{
				name: "all set",
			...
			t.Run(tc.name, func(t *testing.T) {
				var diagnostics diag.Diagnostics
				ds := provider.NewModelToEnterpriseConverter(&diagnostics).RecordingDataSource(tc.src)
				assert.Empty(t, diagnostics)
				assert.Empty(t, cmp.Diff(tc.want, ds, protocmp.Transform()))
			})
		}
	})
}
