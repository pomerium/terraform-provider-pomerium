	...
	t.Run("Route", func(t *testing.T) {
		...
		t.Run("empty", func(t *testing.T) {
			...
			assert.Equal(t, provider.RouteModel{
				...
				EnableGoogleCloudServerlessAuthentication: types.BoolValue(false),
				...
				ID:                                types.StringNull(),
				IdentityProviders:                 types.SetNull(types.StringType),
				...
				MCP:                               types.ObjectNull(provider.RouteMCPObjectType().AttrTypes),
				Name:                              types.StringNull(),
				NamespaceID:                       types.StringNull(),
	...
	t.Run("Settings", func(t *testing.T) {
		...
		t.Run("empty", func(t *testing.T) {
			...
			assert.Equal(t, provider.SettingsModel{
				...
				BearerTokenFormat:                 types.StringNull(),
				BlobStorage:                       types.ObjectNull(provider.BlobStorageSettingsObjectType().AttrTypes),
				...
				DatabrokerStorageConnectionString: types.StringNull(),
				...
				SessionRecordingConcurrency:     types.Int64Null(),
		...
		t.Run("properties", func(t *testing.T) {
			...
			result := provider.NewAPIToModelConverter(&diagnostics).Settings(&configpb.Settings{
				...
				Id:                                new("SETTINGS_ID"),
				IdentityProviders: map[string]*configpb.IdentityProvider{
					"idp1": {
				...
				SessionRecordingConcurrency: new(uint32(4)),
			...
			assert.Equal(t, types.StringValue("DSN"), result.DatabrokerStorageConnectionString)
			...
			assert.Equal(t, types.Int64Value(4), result.SessionRecordingConcurrency)
			assert.Equal(t, types.SetValueMust(types.StringType, []attr.Value{types.StringValue("/etc/envoy/ext1.so"), types.StringValue("/etc/envoy/ext2.so")}), result.EnvoyDynamicExtensions)
