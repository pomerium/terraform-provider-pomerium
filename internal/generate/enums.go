...
func generateAPIToModelEnums(_ context.Context) error {
	...
	buf.WriteString("\n")
	buf.WriteString("\t\"github.com/pomerium/sdk-go/proto/pomerium\"\n")
	buf.WriteString(")\n")
	...
	for _, desc := range enumDescriptors {
		...
		buf.WriteString("\n")
		fmt.Fprintf(&buf, "func (c *APIToModelConverter) %s(src *pomerium.%s) types.String {\n", methodName, typeName)
	...
	return os.WriteFile("./internal/provider/converters_api_to_model.gen.go", buf.Bytes(), 0o0600)
}
...
func generateModelToAPIEnums(_ context.Context) error {
	...
	for _, desc := range enumDescriptors {
		...
		valuePrefix := strcase.ToScreamingSnake(string(desc.Name())) + "_"
		buf.WriteString("\n")
		fmt.Fprintf(&buf, "func (c *ModelToAPIConverter) %s(p path.Path, src types.String) *pomerium.%s {\n", methodName, typeName)
		...
		for i := range desc.Values().Len() {
			...
			buf.WriteString(`		return pomerium.` + typeName + `(` + fmt.Sprint(valueDesc.Number()) + `).Enum()` + "\n")
		}
		buf.WriteString(`	default:` + "\n")
	...
	return os.WriteFile("./internal/provider/converters_model_to_api.gen.go", buf.Bytes(), 0o0600)
}
